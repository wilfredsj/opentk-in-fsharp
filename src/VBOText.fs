...
type VBOTextWindow(w, h, title, message : string) =
  inherit GameWindow(w, h, GraphicsMode.Default, title)
  
  let mutable current_texture = 0

  let nChars = message |> Seq.length

  let charGridSize = 14
  let charsFrom i j n =
    Array.init n (fun idx -> (i, j + idx))
    |> Array.map (
      fun (i', j') ->
        if j' >= charGridSize then
          (i' + (j' / charGridSize), j' % charGridSize)
        else
          (i', j'))

  let vertexCoords = 
    let screenCoordsIJ N z (i : int) (j : int) = 
      // Assume we write everything on [-1,1], [-1,1], [z]
      // with discrete coordinates from 0 to N in each direction
      let scale = 2.0f / (float32 N)
      let xf = j |> float32 |> fun x -> x * scale - 1.0f |> fun x -> -1.0f * x
      let yf = i |> float32 |> fun y -> y * scale - 1.0f |> fun y -> -1.0f * y
      let dx = scale
      let dy = scale
      [| xf + dx; yf + dy; z; xf; yf + dy; z; xf; yf; z; xf + dx; yf; z|] 
    let screenCoordsIJ4 =  screenCoordsIJ charGridSize 4.0f
    charsFrom 7 6 nChars |> Array.collect(fun (i,j) -> screenCoordsIJ4 i j)

  let textureCoords = 
    let textureCoordsIJ i j = 
      let scale = 1.0f / 16.0f
      let xf = j |> float32 |> fun x -> x * scale
      let yf = i |> float32 |> fun x -> x * scale
      let dx = scale
      let dy = scale
      [| xf; yf; xf + dx; yf; xf + dx; yf + dy; xf; yf + dy |] 
    let textureCoordsCh (ch : char) = 
      let idx = ch |> uint32
      let i = idx / 16u
      let j = idx % 16u
      textureCoordsIJ i j
    message |> Seq.collect(textureCoordsCh) |> Array.ofSeq

  let vertexIndicesN n = 
    let myInit = fun n' -> 
      let i = 4 * n' |> uint32
      [| i; i + 1u; i + 2u; i + 3u |]
    Array.init n myInit
    |> Array.collect(id)
  let vertexIndices = vertexIndicesN nChars
  ...
  // Extensions of the above, these two use the () -> unit convention
  ...
  // Below 'loadTexture' function copied from http://deathbyalgorithm.blogspot.com/2013/05    /opentk-textures.html
  ...
  override o.OnRenderFrame e =
    ...
    let drawElts bt = GL.DrawElements(BeginMode.Quads, 4 * nChars, DrawElementsType.UnsignedInt, 0)
        
    let renderFrameGivenCorrectState () = 
      vertexBuffer 
    ...
    // Now generate a function that
    //  i.   sets all the state needed
    //  ii.  calls above renderer
    //  iii. unsets all said state
    let renderFrame = 
      renderFrameGivenCorrectState
      |> execWithClientState ArrayCap.TextureCoordArray
      |> execWithClientState ArrayCap.VertexArray
      |> execWithBoundTexture current_texture
